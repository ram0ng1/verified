...
export default class VerificationRequest extends Model {
  ...
  updatedAt() {
    return Model.attribute<Date | null, string | null>('updatedAt', Model.transformDate).call(this);
  }
  handledAt() {
    return Model.attribute<Date | null, string | null>('handledAt', Model.transformDate).call(this);
  }
  user() {
  ...
  apiEndpoint(): string {
    return '/verification-requests' + ('id' in this.data ? '/' + this.data.id : '');
  }
