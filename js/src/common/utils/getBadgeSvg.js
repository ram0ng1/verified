...
export const DEFAULT_VERIFIED_SVG = `<svg viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="${DEFAULT_SVG_PATH}" fill="currentColor"></path></svg>`;
...
export default function getBadgeSvg() {
  try {
    if (typeof app !== 'undefined' && app.forum) {
      const raw = app.forum.attribute('ramonVerifiedBadgeSvgContent');
      if (typeof raw === 'string' && raw.trim()) {
        ...
        // Setting cleared — drop any cached value so the default takes over.
...
export function getBadgeSize() {
  try {
    if (typeof app !== 'undefined' && app.forum) {
      const raw = app.forum.attribute('ramonVerifiedBadgeSize');
...
export function getBadgeColor() {
  try {
    if (typeof app !== 'undefined' && app.forum) {
      if (!app.forum.attribute('ramonVerifiedCustomColorEnabled')) return null;

      const c = app.forum.attribute('ramonVerifiedBadgeColor');
      if (typeof c === 'string') {
        const v = c.trim();
