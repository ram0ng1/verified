...
import ApprovedUsersState, {
  ...
  ApprovedUserRow,
} from "../states/ApprovedUsersState";
...
export default class VerificationRequestsSection extends Component<ComponentAttrs> {
        ...
        <nav className="VerifiedRequests-tabs" role="tablist">
          {TABS.map((status) => (
            ...
            >
              ...
              <span className="VerifiedRequests-count">
                {counts[status] ?? 0}
              </span>
            </button>
          ))}
  ...
  renderApprovedTab(): Mithril.Children {
    const { loading, rows, total, offset, query, tiers, tierFilter } =
      this.approved;
    ...
    return [
      <div className="VerifiedRequests-toolbar">
        <div className="VerifiedRequests-search">
          ...
          <input
            ...
            value={query}
            oninput={(e: Event) =>
              this.approved.setSearch((e.target as HTMLInputElement).value)
            }
          />
          {query ? (
            <button
      ...
      tiers.length > 0 ? (
        <div
          ...
          role="tablist"
          aria-label={extractText(trans("tier_filter_aria"))}
        >
          <button
            type="button"
            role="tab"
          ...
          {tiers.map((t) => {
            ...
            return (
              <button
                ...
                key={t.id}
                className={
                  "VerifiedRequests-tierChip" + (active ? " is-active" : "")
                }
                style={
                  swatch
                    ? ({ "--tier-color": swatch } as Record<string, string>)
                    : undefined
                }
      ...
      loading && rows.length === 0 ? (
        ...
        <div className="VerifiedRequests-empty">
          <i className="icon fas fa-inbox" />
          <span>
            {query ? trans("empty_search", { query }) : trans("empty_approved")}
          </span>
        </div>
      ) : (
        <ul
          className={
            "VerifiedRequests-list" + (loading ? " is-refreshing" : "")
          }
        >
  ...
  renderApprovedItem(row: ApprovedUserRow): Mithril.Children {
    ...
    const tierSwatch =
      tier && /^#[0-9a-f]{3,8}$/i.test(tier.color) ? tier.color : null;

    return (
      <li
        className={
          "VerifiedRequest" + (isGroupOnly ? " VerifiedRequest--auto" : "")
        }
        key={"user-" + row.id}
      >
        <div className="VerifiedRequest-main">
          <div className="VerifiedRequest-user">
            ...
            {tier ? (
              <span
                ...
                style={
                  tierSwatch
                    ? ({ "--tier-color": tierSwatch } as Record<string, string>)
                    : undefined
                }
                title={tier.label}
              >
          ...
          {isGroupOnly ? (
            ...
            this.renderRequestMeta(row.request)
          ) : null}
        </div>

        <div className="VerifiedRequest-actions">
          {isGroupOnly ? (
            <span className="VerifiedRequest-autoHint helpText">
              {trans("auto_actions_hint")}
            </span>
          ) : (
            <Button
  ...
  renderRequestMeta(req: ApprovedRequestSummary): Mithril.Children {
    ...
    const docUrl = docPath
      ? app.forum.attribute("apiUrl") + "/verified/documents/" + req.id
      : null;
    const created = req.createdAt ? new Date(req.createdAt) : null;
    const handled = req.handledAt ? new Date(req.handledAt) : null;
    const handlerName = req.handler
      ? req.handler.displayName || req.handler.username
      : null;

    return [
      <div className="VerifiedRequest-meta helpText">
        <span title={created ? created.toString() : ""}>
          {humanTime(created)}
        </span>
        {handlerName && handled ? (
          <span>
      ...
      docPath && docUrl ? (
        <div className="VerifiedRequest-document">
          ...
          <span className="VerifiedRequest-docType">
            {this.formatDocType(req.documentType)}
          </span>
          <button
            type="button"
            className="VerifiedRequest-docLink"
            onclick={() =>
              app.modal.show(DocumentPreviewModal, {
                url: docUrl,
                filename: docPath,
  ...
  formatDocType(type: string | null | undefined): string {
    if (!type) return "";
    const configured = app.forum.attribute<
  ...
  renderItem(req: VerificationRequest): Mithril.Children {
    ...
    const docUrl =
      docPath && reqId
        ? app.forum.attribute("apiUrl") + "/verified/documents/" + reqId
        : null;
      ...
      <li className="VerifiedRequest" key={req.id()}>
        <div className="VerifiedRequest-main">
          <div className="VerifiedRequest-user">
            ...
            <span
              className={
                "VerifiedRequest-status VerifiedRequest-status--" + req.status()
              }
            >
          ...
          <div className="VerifiedRequest-meta helpText">
            <span
              title={
                req.createdAt() ? (req.createdAt() as Date).toString() : ""
              }
            >
              {humanTime(req.createdAt())}
            </span>
            {handler &&
            typeof handler.displayName === "function" &&
            req.handledAt() ? (
              <span>
                {" · "}
                {trans("handled_by", {
          ...
          {req.reason() ? (
            <blockquote className="VerifiedRequest-reason">
              {req.reason()}
            </blockquote>
          ) : null}
          ...
          {req.isPending() ? (
            [
              <Button
                ...
                loading={busy}
                onclick={() => this.actOnRequest(req, "approve")}
              >
              ...
              <Button
                className="Button Button--danger"
                loading={busy}
                onclick={() => this.actOnRequest(req, "reject")}
              >
            ...
            <Button
              className="Button Button--danger"
              loading={busy}
              onclick={() => this.actOnRequest(req, "revoke")}
            >
              <i className="icon fas fa-ban" /> {trans("revoke_button")}
            </Button>
          ) : null}
